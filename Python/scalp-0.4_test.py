...
__vrn__ = "0.4"
__release__ = __application__ + '/' + __vrn__ #Changed from __version__
			...
			for _hash in regs[attack_type]:
				if _hash not in done:
					...
					if attack[0].search(cur_line):
						...
						array[attack_type][attack[1]].append(
							(exp_line, attack[3], attack[2], org_line))
...
def scalper(access, filters, preferences=[], output="text"):
	...
	if preferences['sample'] != float(100):
		...
		sampled_lines = random.sample(range(total_nb_lines), int(
			float(total_nb_lines) * preferences['sample'] / float(100)))
		sampled_lines.sort()
		...
		for line in log_file:
			...
			if c_reg.match(line):
				...
				if len(url) > 1 and method in ('GET', 'POST', 'HEAD', 'PUT', 'PUSH', 'OPTIONS'):
					analyzer([(ip, name, date, ext, method, url, response, byte,
                                            referrer, agent), regs, flag, preferences, line])
	...
	# generate exceptions
	if len(diff) > 0:
		o_except = open(os.path.abspath(
			preferences['odir'] + os.sep + "scalp_except.txt"), "w")
		for l in diff:
			o_except.write(l + '\n')
		o_except.close()
